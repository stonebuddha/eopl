...
type bounce =
  | Ans of expval
  | Jump of expression * environment * continuation
...
and apply_procedure_k proc arg_val cont =
  match proc with
  | (body, saved_env) -> Jump (body, extend_env arg_val !saved_env, cont)
...
let rec trampoline b =
  match b with
  | Ans eval -> eval
  | Jump (exp, env, cont) -> trampoline (value_of_k exp env cont)

let value_of_top_level top env =
  match top with
  | ValTop (var, exp1) ->
    let eval1 = trampoline (value_of_k exp1 env EndCont) in
