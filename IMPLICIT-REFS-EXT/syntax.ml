...
EXTEND
  ...
  t : [
    [ exp1 = e; ";" -> fun ctx -> (ValTop ("it", exp1 ctx, loc), extend_ctx "it" ctx)
    | "val"; var = LIDENT; "="; exp1 = e; ";" -> fun ctx -> (ValTop (var, exp1 ctx, loc), extend_ctx var ctx)
    | "fun"; p_name = LIDENT; "(";  b_var = LIDENT; ")"; "="; p_body = e; ";" -> fun ctx -> (FunTop (p_name, p_body (extend_ctx b_var (extend_ctx p_name ctx)), loc), extend_ctx p_name ctx) ]
  ];
  e : [
